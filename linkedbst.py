...
class LinkedBST(AbstractCollection):
    ...
    def height(self):
        ...
        def height1(top):
            ...
            left = top.left
            right = top.right
            cur_height = max(height1(left), height1(right))+1
            return cur_height
    ...
    def is_balanced(self):
        ...
        if self.isEmpty():
            return True
        return self.height() == math.ceil(log(self._size+1)) - 1
    ...
    def range_find(self, low, high):
        ...
        def inorder_range(node, low, high):
            if node is None:
                return []

            if low <= node.data <= high:
                inorder_range(node.left, low, high, result)
                result.append(node.data)
                inorder_range(node.right, low, high, result)
            elif node.data < low:
                inorder_range(node.right, low, high, result)
            else:
                inorder_range(node.left, low, high, result)
        ...
        result = []
        inorder_range(self._root, low, high, result)
        return result
